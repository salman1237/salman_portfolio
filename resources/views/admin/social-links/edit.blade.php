...
@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.social-links.update', $socialLink) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="glass-card rounded-2xl p-6">
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    @if($socialLink->icon)
                        <div class="mt-2 mb-3">
                            ...
                            <img src="{{ asset('storage/' . $socialLink->icon) }}" alt="{{ $socialLink->name }}" class="w-10 h-10 object-contain bg-white/10 rounded p-1">
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-xs text-gray-400">Leave empty to keep current icon. PNG, JPG, SVG, WEBP up to 2MB.</p>
                    @error('icon')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                    ...
                    <div id="iconPreview" class="mt-3 hidden">
                        <p class="text-xs text-gray-400 mb-1">New preview:</p>
                ...
                <div>
                    ...
                    <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $socialLink->display_order) }}"
                        class="mt-1 block w-full rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-3">
